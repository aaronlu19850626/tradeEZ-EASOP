...
> 实施状态说明：本文描述的是后端目标协议。当前独立同步客户端 `tradeEZSync.mq5` 的兼容实现仍按“平仓时间”选择并推进 `last_sync_time`，尚未切换为本文定义的开仓时间游标，更未完成计划中的 `deal_time_msc + deal_ticket` 游标。客户端与服务端完成同一版本联调前，不得把本文当作当前生产行为的证明。
...
同一个 `tradeEZSync.mq5` 实例的 `OnTimer()` 与 `OnChartEvent()` 由 MT5 串行派发。自动同步执行 `WebRequest()` 时，手动点击事件只能等待，不能插入当前同步批次；点击事件恢复处理后仍须经过二次确认。因此，同一实例不会同时运行自动同步和手动同步。

该保证只适用于一个 MT5 程序实例：

- 终端全局变量租约只能约束同一个 MT5 数据目录；
- 同一账户在另一台电脑、另一个 MT5 数据目录或其他客户端发起同步时，仍可能出现并发请求；
- 客户端事件顺序不能替代服务器的一致性控制。

`ingest/deals` 与 `update_last_sync_time` 是两个独立 HTTP 请求，不构成跨请求数据库事务。服务端必须同时满足：

1. 单个成交批次在一个数据库事务中全量成功或全量失败；
2. `(account_id, ticket)` 唯一，重复 ticket 返回成功，不产生重复业务记录；
3. 成交批次未完整持久化时不得返回 `2xx`；
4. 游标更新前验证候选游标对应的数据已经持久化；
5. 使用 `GREATEST(current_cursor, candidate_cursor)` 或等价原子语句单调更新，禁止游标倒退；
6. 相同候选游标重复提交必须成功；
7. 多客户端并发上传、乱序到达或重试时仍满足上述规则。

客户端在任一成交批次失败时不提交游标；若成交已上传而游标请求失败，下一轮会幂等重传成交后再次提交。因此服务器不得依赖“请求只会到达一次”或“手动请求一定晚于自动请求”。
...
15. 自动同步完成后立即手动同步，重复成交不新增业务记录，游标不变或单调前进。
16. 两个不同终端对同一账户同时提交大小不同的候选游标，最终游标等于较大值且不倒退。
17. 成交批次数据库事务回滚时返回非 `2xx`，游标保持不变。
18. 成交持久化成功但游标请求超时，重试后成交不重复且游标可以正常推进。
